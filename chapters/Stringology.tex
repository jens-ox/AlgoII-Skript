...
\begin{pseudocode}
  ...
  \phantom{\quad} \textbf{if} \( \text{SA}^{-1}[i] \neq 1 \) \textbf{then} \\
  \phantom{\quad} \phantom{\quad} \textbf{while} \( t_{i+h} = t_{\text{SA}[\text{SA}^{-1}[i]-1]+h} \) \textbf{do} \( h \)++ \\
  \phantom{\quad} \phantom{\quad} \( \text{LCP}[\text{SA}^{-1}[i]] \coloneqq h \) \\
  \phantom{\quad} \phantom{\quad} \( h \coloneqq \max(0,h-1) \)
\end{pseudocode}
...
\begin{figure}[H]
  \includegraphics[width=0.8\textwidth]{buildSuffixTree}
...
Eine Anwendung der Suffix-Arrays und -Trees ist die \term{Datenkompression}\index{Datenkompression}.